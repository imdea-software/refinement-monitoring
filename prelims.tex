...
operation}. An $m(u)=>v$ operation is called \emph{completed} if $v\neq\bot$ and
\emph{pending}, otherwise.
A history with only completed operations is called \emph{complete}. 
The histories admitted by $L$ are $H(L) = \set{ H(e) : e \in L}$. 
...
\in H$ as well. Also, we consider only histories of well-formed executions 
without explicit qualification.

The first property is that libraries are closed under the weakening relation
$\preceq$.

\begin{lemma}
  \label{lemma:lib:downward_closed}
