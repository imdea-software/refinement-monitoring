...
enables the construction of a formula  $\mathit{Stronger}(h)$ characterizing all the histories
stronger than $h$. This formula conjuncted with $\psi$ describes all the histories stronger than $h$
that satisfy $\psi$. Therefore, the satisfiability of $\mathit{Stronger}(h)\land \psi$ is
equivalent to $h\in H(L)$. 
When the history $h$ contains pending operations, a stronger history may contain
less operations (pending operations of $h$ can be omitted in a stronger history). 
Therefore, the formula $\mathit{Stronger}(h)$ uses a domain predicate $\<Ops>$
defining the set of operations in the stronger history. The histories stronger
than $h$ which satisfy $\psi$ are described by the formula $\mathit{Stronger}(h)\land \psi'$
where $\psi'$ is obtained from $\psi$ by guarding the quantified variables to satisfy
the domain predicate $\<Ops>$.
...
Given a complete history $h=\tup{O,<,f}$, let $\mathit{Stronger}(h)$ be a formula that characterizes
...
\mathit{Stronger}(h)\ \triangleq\ & \forall x.\ x\in O\ \ \land \<Distinct>(O) \\
		    ...
		    & \hspace{-10mm}\land\ \hspace{-7mm}\bigwedge_{\scriptsize \begin{array}{c} f(o)=m(u)=>v\end{array}} \hspace{-7mm} \<meth>(o)=m\land \<arg>(o)=u\land \<ret>(o)=v 
...
The following result states that $\mathit{Stronger}(h)$ characterizes precisely
...
\begin{align*}
h\preceq h'\text{ iff }h'\models \mathit{Stronger}(h).
\end{align*}
...
Then,
\[
h\in H(L)\mbox{ iff }\mathit{Stronger}(h)\land \psi\mbox{ is satisfiable.}
\]
\end{theorem}
%\begin{proof}
...
the satisfiability of $\mathit{Stronger}(h)\land \psi$ 
can be reduced to propositional satisfiability.
This is based on the fact that $\mathit{Stronger}(h)$ fixes the domain of the operation-identifier variables 
...
Therefore, one can construct a propositional formula $\mathit{Bool}(h,\psi)$, which
is equi-satisfiable to $\mathit{Stronger}(h)\land \psi$. The construction of
$\mathit{Bool}(h,\psi)$ can be done in polynomial time provided that the number of
...
%operations is a refinement violation, 
The result in Corollary~\ref{cor:completions} 
leads to an exponential blow-up when checking history membership:
 the number of possible completions of a given history $h$
...
Essentially, this works by constructing a formula which soundly approximates the ``weaker than''
relation between histories and the membership of a history to a basis.
%is weaker than all the first-order formulas
%corresponding to completions of $h$ via Theorem~\ref{th:satisfiability}. 
Therefore, if this formula
is unsatisfiable, then there exists no history stronger than $h$ which is included in the basis
and $h\not\in H(L)$.

\subsubsection{Prefix-closed history sets}

The soundness of this approximation relies on a prefix-closure property of the basis. When the
basis consists of sequential histories this property coincides with the standard prefix closure, i.e., any
prefix of the sequence of operations in a history $h\in H(B)$ is also included in $H(B)$. When
$H(B)$ contains non-sequential histories the prefix-closure property is more intricate since
a poset may have multiple minimal prefixes that include a given set of elements (a prefix of a poset 
$(P,<)$ is a poset $(P',<'$) with $P'\subseteq P$ and $P'$ closed under $<$).

A \emph{prefix} of a history $h=\tup{O,<,f}$ is a history $h'=\tup{O',<',f'}$ over 
a subset of operations of $h$, i.e., $O'\subseteq O$, $<'$ is the projection of $<$ over $O'\times O'$,
and $f'$ is the projection of $f$ over $O'$, that is 
closed under $<$, i.e., $o\in O'$ and $o'<o$ implies $o'\in O'$.
Given a set of operations $O_1$ in $h$, the prefix $h'$ \emph{overlaps} with $O_1$ iff
every operation in $h'$ is ordered before or overlaps with an operation in $O_1$, i.e.,
for every $o'\in O'$ there exists $o\in O_1$ s.t. $\neg o < o'$.

\begin{example}

Prefix of a history.

Multiple minimal prefixes that include a given set of operations.

A prefix overlaps a set of operations

\end{example}

Given a history $h=\tup{O,<,f}$, let $O_c$ be the set of completed operations
in $h$. An operation $o\in O$ is called \emph{obsolete} iff there exists no pending 
operation $o'\in O$ which overlaps with $o$, i.e., for all $o'\in O$, $\neg o<o'$ and $\neg o'<o$ implies
$o'\in O_c$. The set of obsolete operations in the history $h$ is denoted by $O_{ob}$.
Note that every operation of a complete sequential history is obsolete.
...
\begin{definition}\label{lemma:kernel_histories_prefix}

A set of histories $H$ is \emph{prefix-closed} when for every history $h=\tup{O,<,f}\in H$
and every set of obsolete operations $O'\subseteq O$, $H$ contains a prefix of $h$
that includes $O'$ and overlaps with $O'$.

\end{definition}

Since every operation of a complete sequential history is obsolete, a set of complete
sequential histories is prefix-closed whenever ...

The following result follows from the fact that a library $L$ is a prefix-closed sets of executions.

\begin{lemma}

Let $L$ be a library with a prefix-closed basis that consists only of complete histories. 
Then, $H(L)$ is prefix-closed.
...
\subsubsection{Reduction to propositional SAT}
