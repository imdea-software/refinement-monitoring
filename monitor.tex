...
A \emph{match} of a history $h$ is a completed operation $o$ together with the maximal 
set of operations mapped by $\<Matching>(h)$ to $o$. Also, a match must include
only completed operations and at least one operation mapped to $o$. Formally, a match of a 
...
at later times. Therefore, in the continuation of Theorem~\ref{th:match_closure},
one must prove that a match of  
a history $h$ is a match of any extension of $h$.
This result holds when the matching function of all histories in the library is injective,
which is true for all the reference implementations in Section~\ref{sec:logic}
