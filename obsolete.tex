...
%
%even this strategy is incomplete.
...
extension of the former. Removing the match $\{{\tt push}(1),{\tt pop}=>1\}$ from $h_1$
...
that $\<push>(2)$ is ordered before $\<pop>=>Empty$ in any history stronger than $h_1$

the constraint ${\sf b}(\<push>(2), \<pop>=>Empty)$ implied by the presence of this match, 
then, he would be able to detect the violation.
%To detect this violation, the monitor should remember the constraint
