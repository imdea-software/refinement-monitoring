...
\begin{tikzpicture}[node distance=4mm]
  ...
  \draw[opline] (x1) -- node[draw=none,above] {add$(1)$} (x2); 
    
  % pop(1)
  \node[lnode] (x3)  [right = 7mm of x2] {};
  \node[lnode] (x4)  [right= 6mm of x3] {}; 
  \draw[opline] (x3) -- node[draw=none,above] {rem$\Rightarrow 1$} (x4); 

  % pop(-)
  \node[lnode] (x5)  [right = 3mm of x4] {};
  \node[nnode] (x6)  [right= 50mm of x5] {}; 
  \draw[opline] (x5) -- node[draw=none,above] {rem$\Rightarrow \perp$} (x6); 

  % push(2)
  \node[lnode] (y1)  [below = 7mm of x3,xshift=3mm] {};
  \node[lnode] (y2)  [right= 10mm of y1] {}; 
  \draw[opline] (y1) -- node[draw=none,above] {add$(2)$} (y2); 

  % pop(2)
  \node[lnode] (y3)  [right = 13mm of y2] {};
  \node[lnode] (y4)  [right=of y3] {}; 
  \draw[opline] (y3) -- node[draw=none,above] {rem$\Rightarrow 2$} (y4); 

  % push(3)
  \node[lnode] (z1)  [below = 6mm of y2,xshift=4mm] {};
  \node[lnode] (z2)  [right=of z1] {}; 
  \draw[opline] (z1) -- node[draw=none,above] {add$(3)$} (z2); 

  % push(4)
  \node[lnode] (t1)  [below = 13mm of y1,xshift=2mm] {};
  \node[lnode] (t2)  [right=35mm of t1] {}; 
  \draw[opline] (t1) -- node[draw=none,above] {add$(4)$} (t2); 

%  % dotted barrier
%  \node[nnode] (b1)  [right = 2mm of t2,yshift = 25mm] {};
...
%  \draw[dotted] (b1) -- (b2); 
  ...
  \draw[opline] (z3) -- node[draw=none,above] {rem$\Rightarrow 3$} (z4); 
