...
\begin{tikzpicture}[node distance=4mm]
  ...
  \draw[opline] (x3) -- node[draw=none,above] {$\<rem>=>1$} (x4); 

  % pop(-)
  \node[lnode] (x5)  [right = 3mm of x4] {};
  \node[nnode] (x6)  [right= 50mm of x5] {}; 
  \draw[opline] (x5) -- node[draw=none,above] {$\<rem>=>\bot$} (x6); 
  ...
  \draw[opline] (y3) -- node[draw=none,above] {$\<rem>=>2$} (y4); 
...
%  \draw[dotted] (b1) -- (b2); 
  ...
  \draw[opline] (z3) -- node[draw=none,above] {$\<rem>=>3$} (z4); 
