...
\begin{tikzpicture}[node distance=4mm]
  ...
  \draw[opline] (x1) -- node[draw=none,above] {rem $\Rightarrow {\tt empty}$} (x2); 
  ...
  \draw[opline] (y3) -- node[draw=none,above] {rem $\Rightarrow 1$} (y4); 
  ...
  \draw[opline] (z3) -- node[draw=none,above,xshift=-4mm] {rem $\Rightarrow 2$} (z4); 

  % dotted barrier
  \node[nnode] (b1)  [right = 6mm of x2,yshift = 3mm] {};
  \node[nnode] (b2)  [right=  6mm of x2,yshift = -18mm] {}; 
  \node[nnode] (b3)  [above=  2mm of b1] {$h_1$}; 
  \draw[dotted] (b1) -- (b2); %node[draw=none,above] {$\<push>(4)$} 

  % push(3)
  \node[lnode] (t1)  [right = 5mm of y4,xshift=0mm] {};
  ...
  \draw[opline] (t3) -- node[draw=none,above] {rem $\Rightarrow 3$} (t4); 
